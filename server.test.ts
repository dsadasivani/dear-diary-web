...
const createDist = async () => {
  ...
  await writeFile(path.join(distPath, "index.html"), "<!doctype html><title>Dear Diary</title><div id=\"root\"></div>");
...
test("oversized JSON bodies are rejected with 413", async () => {
  const app = await createApp({ mode: "production", distPath: await createDist(), jsonLimit: "1kb" });
