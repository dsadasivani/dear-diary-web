...
import { ALLOWED_OUTBOX_V2_TRANSITIONS, assertAllowedOutboxTransition, isAllowedOutboxTransition } from './OutboxStateMachine';
...
test('accepts every declared outbox transition and rejects every undeclared transition', () => {
  for (const from of OUTBOX_V2_STATES) {
    for (const to of OUTBOX_V2_STATES) {
      ...
      else assert.throws(() => assertAllowedOutboxTransition(from, to), (error: unknown) => (
        error instanceof SyncError && error.code === 'INVARIANT_VIOLATION'
      ));
    }
  }
});
