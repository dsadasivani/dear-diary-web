...
test('persists operations and recovers an expired lease after restart', async () => {
  ...
  assert.equal((await first.claimNextRunnable({ accountId: 'account-1', workerId: 'worker-a', now: 10, leaseDurationMs: 10 }))?.leaseOwner, 'worker-a');

  const restarted = new PersistentOutboxRepository(store);
  assert.equal(await restarted.claimNextRunnable({ accountId: 'account-1', workerId: 'worker-b', now: 19, leaseDurationMs: 10 }), null);
...
test('concurrent workers cannot claim the same operation', async () => {
  ...
  const claims = await Promise.all([
    repository.claimNextRunnable({ accountId: 'account-1', workerId: 'worker-a', now: 10, leaseDurationMs: 50 }),
    repository.claimNextRunnable({ accountId: 'account-1', workerId: 'worker-b', now: 10, leaseDurationMs: 50 }),
  ]);
...
test('dependency operations block claims until acknowledged', async () => {
  ...
  const parent = await repository.claimNextRunnable({ accountId: 'account-1', workerId: 'worker', now: 10, leaseDurationMs: 50 });
  ...
  assert.equal((await repository.claimNextRunnable({ accountId: 'account-1', workerId: 'worker', now: 11, leaseDurationMs: 50 }))?.operationId, 'child');
});

test('compare-and-set transition rejects stale and invalid state changes', async () => {
  const repository = new PersistentOutboxRepository(new MemoryStore());
  await repository.enqueue(operation('op-1'));
  await assert.rejects(repository.transition('op-1', 'PREPARING', 'UPLOADING'), (error: unknown) => error instanceof SyncError && error.code === 'INVARIANT_VIOLATION');
