...
test('recent-first restore imports core and recent monthly partitions from manifest', async () => {
  ...
  const recent = await encryptSyncPayload(
    rootKey,
    'partition_snapshot',
    encodePartitionSnapshotPayload(fullSnapshot, 'account-1', 'month:2026-07', 5),
  );
  const old = await encryptSyncPayload(
    rootKey,
    'partition_snapshot',
    encodePartitionSnapshotPayload(fullSnapshot, 'account-1', 'month:2021-03', 6),
  );
  const manifestPayload = buildPartitionManifest({
    ...
    snapshotMetadata: {
      ...
      'month:2026-07': {
        ...
        latestSnapshotDriveFileId: 'drive-recent',
        latestSnapshotSha256: recent.sha256,
        latestSnapshotSizeBytes: recent.bytes.byteLength,
        headSequence: 5,
      },
      'month:2021-03': {
        latestSnapshotSequence: 6,
        latestSnapshotDriveFileId: 'drive-old',
        latestSnapshotSha256: old.sha256,
        latestSnapshotSizeBytes: old.bytes.byteLength,
        headSequence: 6,
      },
    },
  });
  ...
  const controlPlane = {
    getLatestRestoreManifest: async () => ({
      manifestObject: metadata(
        ...
        'manifest',
        manifest.sha256,
        manifest.bytes.byteLength,
        'core',
      ),
      coreSnapshotObject: metadata(
        4,
        'drive-core',
        'partition_snapshot',
        core.sha256,
        core.bytes.byteLength,
        'core',
      ),
  ...
  assert.deepEqual(
    (await repository.listEntries()).map((entry) => entry.title),
    ['Recent'],
  );
