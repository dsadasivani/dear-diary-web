...
import { decodeRecoveryKeyPackage, unwrapAccountRootKeysFromRecovery } from './e2eeKeyPackage';
...
export const recoverAccountRootKey = async (input: {
  ...
  if (candidates.length === 0)
    throw new Error('No recovery key package was found for this account.');
  ...
  for (const object of candidates) {
    try {
      ...
      const unwrapped = await unwrapAccountRootKeysFromRecovery(
        keyPackage,
        input.recoveryPassphrase,
      );
      const packageEpoch = keyPackage.keyEpoch || object.keyEpoch || 1;
      accountRootKeys[packageEpoch] =
        unwrapped.accountRootKeys[keyPackage.keyEpoch || 1] || unwrapped.accountRootKey;
