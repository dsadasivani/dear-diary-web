...
test('begins and finalizes primary recovery through two-phase RPCs', async () => {
    ...
    fetchImpl: async (input, init: RequestInit = {}) => {
      ...
      return new Response(
        JSON.stringify({
          ...
          revoked_devices: [
            { ...device, id: 'old-primary', activation_state: 'active', revoked_at: '' },
          ],
        }),
        { status: 200, headers: { 'Content-Type': 'application/json' } },
      );
  ...
  assert.equal(
    calls[0].url,
    'https://example.supabase.co/rest/v1/rpc/begin_primary_mobile_recovery',
  );
  assert.equal(
    calls[0].body,
    JSON.stringify({
  ...
  assert.equal(
    calls[1].url,
    'https://example.supabase.co/rest/v1/rpc/finalize_primary_mobile_recovery',
  );
  assert.equal(
    calls[1].body,
    JSON.stringify({
...
test('begins and finalizes device key rotation through two-phase RPCs', async () => {
  ...
  assert.equal(
    calls[1].url,
    'https://example.supabase.co/rest/v1/rpc/finalize_device_key_rotation',
  );
  assert.equal(
    calls[1].body,
    JSON.stringify({
...
test('surfaces Supabase RPC errors with status and detail', async () => {
  const client = new SupabaseControlPlaneClient({
    ...
    anonKey: 'anon-key',
    fetchImpl: async () =>
      new Response(JSON.stringify({ message: 'Device access denied.', code: 'device_revoked' }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' },
      }),
  });
        ...
        JSON.stringify([
          {
            ...
            account_id: 'account-1',
            sequence: 1,
            drive_file_id: 'key-file',
            object_kind: 'key_package',
            sha256: 'a'.repeat(64),
            size_bytes: 100,
            created_by_device_id: 'device-1',
            created_at: '',
    ...
    return new Response(
      JSON.stringify({
        session: {
          ...
          requested_display_name: 'Chrome',
          requested_platform: 'web',
          pairing_code_hash: 'a'.repeat(64),
          expires_at: '2026-07-05T00:10:00.000Z',
          approved_by_primary_device_id: 'primary-1',
          approved_at: '2026-07-05T00:01:00.000Z',
          approved_device_id: 'device-2',
          key_package_drive_file_id: 'drive-key-1',
          key_package_sha256: 'b'.repeat(64),
        ...
        key_object: {
          ...
          account_id: 'account-1',
          sequence: 9,
          drive_file_id: 'drive-key-1',
          object_kind: 'key_package',
          sha256: 'b'.repeat(64),
          size_bytes: 321,
          created_by_device_id: 'primary-1',
          created_at: '',
          key_epoch: 4,
        },
      }),
