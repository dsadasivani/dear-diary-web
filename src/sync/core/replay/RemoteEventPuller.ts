...
import type { DecryptedSyncEvent, SyncReplayStore } from './PersistentReplayStore';
...
export interface SyncEventDecryptor {
  hasKeyEpoch(keyEpoch: number): Promise<boolean>;
  decrypt(bytes: Uint8Array, keyEpoch: number): Promise<DecryptedSyncEvent>;
}
...
export class RemoteEventPuller {
  ...
  constructor(
    private readonly api: Pick<SyncApiClient, 'pullEvents' | 'acknowledgeCursor'>,
    private readonly transfer: BoundedObjectTransfer,
    private readonly decryptor: SyncEventDecryptor,
    private readonly replay: SyncReplayStore,
  ...
  private validateEnvelope(event: SyncRemoteEvent): void {
