...
import { syncObjectKeyForId } from './SyncMediaPreparer';

export class SyncMediaHydrator {
  private readonly inFlight = new Map<string, Promise<string>>();

  constructor(
    private readonly api: Pick<SyncApiClient, 'getMediaDownload'>,
  ...
  private async hydrateOnce(reference: string): Promise<string> {
    ...
    const objectKey =
      stored?.driveFileId || (await syncObjectKeyForId(this.accountId, instruction.objectId));
