...
type Command =
  'WRITE' | 'DELETE' | 'OFFLINE' | 'RECONNECT' | 'CRASH' | 'DUPLICATE' | 'PULL' | 'REVOKE';
interface Event {
  sequence: number;
  operationId: string;
  recordId: string;
...
interface Device {
  ...
  revoked: boolean;
  cursor: number;
  records: Map<string, { version: number; value: number }>;
  localWrites: number;
}
    ...
    fc.property(
      fc.array(
        fc.record({
          ...
          command: fc.constantFrom<Command>(
            'WRITE',
            'DELETE',
            'OFFLINE',
            'RECONNECT',
      ...
      (commands) => {
        ...
        const pull = (device: Device) => {
          ...
          events
            .filter((event) => event.sequence > device.cursor)
            .forEach((event) => {
              ...
              if (event.value === null) device.records.delete(event.recordId);
                ...
                device.records.set(event.recordId, { version: event.version, value: event.value });
        ...
        commands.forEach((command) => {
          ...
          if (command.command === 'RECONNECT' && !device.revoked) {
            device.online = true;
            pull(device);
          }
          ...
          if (command.command === 'WRITE' || command.command === 'DELETE') {
            ...
            events.push({
              ...
              recordId,
              version,
              value: command.command === 'DELETE' ? null : command.value,
            });
            pull(device);
          }
          if (command.command === 'DUPLICATE' && events.length > 0)
            committed.add(events[events.length - 1].operationId);
        });
        ...
        active
          .slice(1)
          .forEach((device) => assert.deepEqual([...device.records], [...active[0].records]));
