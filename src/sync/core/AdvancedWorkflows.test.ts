...
import type { SyncPairing, SyncRotation } from './api/SyncApiTypes';
...
const pairingResponse = (patch: Partial<SyncPairing> = {}): SyncPairing => ({
  accountId: 'account-1',
  pairingId: 'pairing-1',
...
test('recovery never persists the key before the crash-safe fault boundary', async () => {
  ...
  const crashing = new SyncRecoveryCoordinator(
    api,
    journal,
    local,
  ...
  await new SyncRecoveryCoordinator(api, journal, local).run('device-1', 'test');
...
test('rotation resumes after authoritative epoch commit without committing it twice', async () => {
  const journal = new MemoryJournal<any>();
  let status: SyncRotation['status'] = 'PREPARING';
  let epochCommits = 0;
  let localCommits = 0;
  let revokedDeviceId: string | null = null;
  const rotation = (): SyncRotation => ({
  ...
  const api = {
    ...
    async advanceRotation(_id: string, _device: string, next: SyncRotation['status']) {
      status = next;
      return rotation();
    },
  ...
  const crashing = new SyncRotationCoordinator(
    api,
    journal,
    local,
  ...
  await new SyncRotationCoordinator(api, journal, local).run('device-1', 'companion-1');
  assert.equal(epochCommits, 1);
