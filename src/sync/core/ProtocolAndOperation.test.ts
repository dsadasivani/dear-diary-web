...
test('protocol bootstrap uses cached remote-pull flags but disables writes when refresh fails', async () => {
  ...
  await controls.save(protocol());
  const runtimeStore = new SyncRuntimeStore(store);
...
test('revoked companion cleanup removes every account-bound sync cache before re-pairing', async () => {
  const store = new MemoryDataStore();
  await Promise.all(
    SYNC_LOCAL_CACHE_KEYS.map((key) => store.setItem(key, JSON.stringify({ stale: true }))),
  );
  await store.setItem('unrelated-preference', 'keep');

  await clearSyncLocalCache(store);

  for (const key of SYNC_LOCAL_CACHE_KEYS) assert.equal(await store.getItem(key), null);
...
test('unknown pull safety stop can retry without clearing integrity stops', async () => {
  const store = new MemoryDataStore();
...
const operation = (patch: Partial<SyncOperation> = {}): SyncOperation => ({
...
test('runtime coordinator starts only workers allowed by bootstrap', async () => {
  const calls: string[] = [];
  const coordinator = new SyncRuntimeCoordinator(
    {
      initialize: async () => ({
        runtime: runtime(),
        protocol: protocol(),
