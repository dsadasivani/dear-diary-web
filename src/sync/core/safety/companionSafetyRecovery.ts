...
export const clearRecoverableCompanionSafetyStop = async (
  ...
  const runtimeRaw = await store.getItem('deardiary_sync_account');
  if (!runtimeRaw) return false;
  const runtime = JSON.parse(runtimeRaw) as { accountId?: string; appliedSequence?: number };
  if (runtime.accountId !== account.accountId || runtime.appliedSequence !== 0) return false;
