...
test('lazy migration uploads monthly partition snapshots and a manifest', async () => {
  ...
  const diary = await repository.createDiary({ name: 'Travel', emoji: 'T', color: '#123456', isLocked: false });
  await repository.createEntry({
    diaryId: diary.id, date: '2026-07-04', title: 'Recent', body: '', moodName: 'Calm',
    moodEmoji: '', tags: [], photoUris: [],
  });
  await repository.createEntry({
    diaryId: diary.id, date: '2021-03-02', title: 'Archive', body: '', moodName: 'Calm',
    moodEmoji: '', tags: [], photoUris: [],
  });
  const localState = {
    accountId: 'account-1',
  ...
  const parsedPartition = parsePartitionSnapshotPayload(decryptedPartition.payload, 'account-1', 'month:2026-07');
  assert.deepEqual(parsedPartition.snapshot.entries.map(entry => entry.title), ['Recent']);
  ...
  assert.deepEqual(manifest.partitions.map(partition => partition.partitionKey), [
    'core',
    'month:2026-07',
    'month:2021-03',
  ]);
