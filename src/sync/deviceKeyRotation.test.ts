...
class FakeControlPlane implements DeviceKeyRotationControlPlane {
  ...
  async beginDeviceKeyRotation(input: {
    primaryDeviceId: string;
    deviceId: string;
    reason: string;
  }): Promise<KeyEpochRotation> {
    if (this.rotation?.status === 'pending')
      throw new Error('duplicate key violates unique pending rotation');
    this.rotation = {
      id: 'rotation-1',
      accountId: 'account-1',
  ...
  async finalizeDeviceKeyRotation(input: {
    ...
    revocation: { accountId: string; deviceId: string; reason: string; createdAt: string };
  }> {
    ...
    if (
      !this.rotation ||
      this.rotation.id !== input.rotationId ||
      this.rotation.status !== 'pending'
    ) {
  ...
  async updateDeviceCursor(input: {
    deviceId: string;
    lastAppliedSequence: number;
  }): Promise<SyncDeviceCursor> {
...
const setupPrimaryAccount = async () => {
  ...
  const recoveryBytes = encodeRecoveryKeyPackage(
    await wrapAccountRootKeyForRecovery(rootKey, 'a durable recovery passphrase', {
      accountId: 'account-1',
      keyVersion: 1,
    }),
  );
...
const createRecordingUpload = (uploads: Map<string, Uint8Array>) => {
  ...
  return async (input: UploadDriveSyncObjectInput): Promise<DriveSyncObjectSummary> => {
    nextUpload += 1;
    const id =
      input.appProperties?.purpose === 'recovery'
...
test('resume completes rotation after recovery package was committed', async () => {
  ...
  await assert.rejects(
    () =>
      revokeDeviceWithKeyRotation({
        ...
        upload: async (input) => {
          if (input.appProperties?.targetDeviceId)
            throw new Error('simulated process death after recovery package');
          return upload(input);
        },
        now: () => 2_000,
      }),
  ...
  const recoveryKeys = await unwrapAccountRootKeysFromRecovery(
    recoveryPackage,
  ...
  assert.equal(
    fixture.controlPlane.devices.find((device) => device.id === fixture.target.id)?.revokedAt !==
      null,
    true,
  );
  ...
  assert.equal(
    (await fixture.repository.getLocalSyncAccountState())?.recoveryKeyDriveFileId,
    'drive-recovery-1',
  );
  ...
  assert.equal(
    fixture.controlPlane.objects.some(
      (object) => object.operationId === `key-epoch:account-1:2:rotation-1:${fixture.survivor.id}`,
    ),
    true,
  );
