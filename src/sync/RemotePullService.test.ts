...
const state = (partitionedSyncEnabled: boolean): LocalSyncAccountState => ({
  ...
  currentSyncSequence: 3, partitionedSyncEnabled, linkedAt: 1,
});
...
const createService = (repository: DiaryRepository) => new RemotePullService(repository, {
...
test('partitioned pull repairs a recent event-only partition from existing local records', async () => {
  ...
  const repository = {
    ...
    getPartitionHydrationState: async (partitionKey: string) => hydration.get(partitionKey) || {
      partitionKey, status: 'not_available', lastAppliedSequence: 0,
    },
    listAvailableArchiveMonths: async () => Array.from(hydration.values())
      .filter(item => item.partitionKey !== 'core'),
    ...
    exportSnapshot: async () => ({
      entries: [{ date: '2026-07-05' }], notes: [],
    } as unknown as RepositorySnapshot),
  } as unknown as DiaryRepository;
  const controlPlane = {
    listSyncObjectsAfter: async () => [],
    listPartitionHeads: async () => [{
      partitionKey: 'month:2026-07', latestEventSequence: 8, latestSnapshotSequence: 0,
    }],
