...
const pairingResponse = (patch: Partial<SyncV2Pairing> = {}): SyncV2Pairing => ({
  accountId: 'account-1', pairingId: 'pairing-1', requestedDeviceId: 'device-2',
  requestedDeviceEncryptionPublicKey: 'encryption-public', platform: 'web', challenge: 'challenge',
  status: 'REQUESTED', keyEpoch: 1, keyPackageId: null, objectKey: null, sha256: null,
  sizeBytes: null, downloadUrl: null, downloadExpiresAt: null, upload: null,
  requestedAt: new Date(0).toISOString(), expiresAt: new Date(60_000).toISOString(), ...patch,
});
...
test('pairing request sends distinct signing and encryption public keys', async () => {
  ...
  const coordinator = new SyncV2PairingCoordinator({
    async createPairing(request: Record<string, unknown>) { created = request; return pairingResponse(); },
  } as any, journal, {
    async createDeviceKey() {
      return { signingPublicKey: 'signing-public', encryptionPublicKey: 'encryption-public', privateKeyHandle: 'private' };
    },
    async randomChallenge() { return 'challenge'; }, async sign() { return 'signature'; },
    async approvalSignature() { return 'signature'; }, async encryptKeyPackage() { return new Uint8Array([1]); },
    async decryptAndPersist() {},
  }, { async upload() {}, async download() { return new Uint8Array([1]); } }, new MemoryJournal<any>());
...
test('pairing completion retains its secure journal until local snapshot activation succeeds', async () => {
  const journal = new MemoryJournal<any>();
  journal.value = {
    pairingId: 'pairing-1', requestedDeviceId: 'device-2', privateKeyHandle: 'private',
    pairingCode: '12345678', challenge: 'challenge',
  };
  let completionCalls = 0;
  const coordinator = new SyncV2PairingCoordinator({
    async getPairing() {
      return pairingResponse({ status: 'KEY_PACKAGE_AVAILABLE', keyPackageId: 'package-1',
        downloadUrl: 'memory://package', sha256: 'a'.repeat(64), sizeBytes: 1 });
    },
    async completePairing() { completionCalls += 1; return pairingResponse({ status: 'COMPLETED', keyPackageId: 'package-1' }); },
  } as any, journal, {
    async createDeviceKey() { throw new Error('unused'); }, async randomChallenge() { return 'unused'; },
    async sign() { return 'signature'; }, async approvalSignature() { return 'signature'; },
    async encryptKeyPackage() { return new Uint8Array([1]); }, async decryptAndPersist() {},
  }, { async upload() {}, async download() { return new Uint8Array([1]); } }, new MemoryJournal<any>());
  await assert.rejects(() => coordinator.complete(async () => { throw new Error('snapshot import interrupted'); }));
...
test('migration keeps its journal until local V2 activation succeeds', async () => {
  ...
  const response = () => ({ migrationId: 'migration-activation', status, baselineDigest: 'b'.repeat(64),
    validationDigest: null, baselineSequence: 4, activatedSequence: status === 'V1_READ_ONLY' ? 0 : null,
    snapshotId: 'snapshot-activation', v1Mode: status === 'V1_READ_ONLY' ? 'READ_ONLY' : 'READ_WRITE' });
  const api = {
    async beginMigration() { return response(); },
    async advanceMigration(_id: string, request: { nextStatus: SyncV2MigrationStatus }) { status = request.nextStatus; return response(); },
    async getMigration() { return response(); },
  } as any;
  const local = {
    async drainV1() {}, async canonicalDigest() { return 'b'.repeat(64); }, async baselineSequence() { return 4; },
    async createSnapshot() { return { snapshotId: 'snapshot-activation' }; }, async verifyTemporaryRestore() { return 'b'.repeat(64); },
    async activateV2() { if (activationAttempts++ === 0) throw new Error('local persistence interrupted'); },
  };
...
test('recovery never persists the key before the crash-safe fault boundary', async () => {
  ...
  const local = {
    async createDeviceKey() {
      return {
        publicKey: 'public', signingPublicKey: 'signing-public',
        encryptionPublicKey: 'encryption-public', privateKeyHandle: 'secure-handle',
      };
    },
...
test('rotation resumes after authoritative epoch commit without committing it twice', async () => {
  ...
  const rotation = (): SyncV2Rotation => ({ rotationId: 'rotation-1', initiatedByDeviceId: 'device-1', revokedDeviceId,
    fromKeyEpoch: 1, toKeyEpoch: 2, status });
  const api = {
    async beginRotation(_id: string, _device: string, revoked?: string) { revokedDeviceId = revoked || null; return rotation(); },
    async getRotation() { return rotation(); },
