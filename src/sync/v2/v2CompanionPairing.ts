...
import { exportDeviceSigningPublicKeySpki, generateDeviceKeyPair, parseDevicePrivateKeyBundle } from '../deviceKeys';
...
import { SyncV2PairingCoordinator, type WorkflowJournalStore } from './advanced/AdvancedWorkflowCoordinators';
...
class SecretJournal<T> implements WorkflowJournalStore<T> {
  ...
  load(): Promise<T | null> { return this.loadValue(); }
  save(value: T): Promise<void> { return this.saveValue(value); }
  clear(): Promise<void> { return this.clearValue(); }
}

const requestJournal = () => new SecretJournal<PairingJournal>(
  loadPendingV2PairingSecret,
  savePendingV2PairingSecret,
  clearPendingV2PairingSecret,
);

const approvalJournal = () => new SecretJournal<PairingApprovalJournal>(
  loadPendingV2PairingApprovalSecret,
  savePendingV2PairingApprovalSecret,
  clearPendingV2PairingApprovalSecret,
);
...
export const signWithDeviceBundle = async (privateKeyBundle: string, message: string): Promise<string> => {
  ...
  const signature = ecdsaP1363ToDer(new Uint8Array(await crypto.subtle.sign(
...
const transferAdapter = (maximumBytes: number) => {
  ...
  return {
    upload: async (bytes: Uint8Array, instruction: Parameters<BoundedObjectTransfer['upload']>[1][number]) => {
    ...
    download: async (pairing: Pick<SyncV2Pairing, 'downloadUrl' | 'sha256' | 'sizeBytes'>) => {
      if (!pairing.downloadUrl || !pairing.sha256 || !pairing.sizeBytes) throw new Error('Pairing package metadata is incomplete.');
...
const repositorySnapshotFromV2 = (state: SyncV2CanonicalSnapshotState): RepositorySnapshot => {
  const values = (prefix: string) => Object.entries(state.records)
...
const pairingCrypto = (options: {
  ...
  onUnwrapped?: (result: Awaited<ReturnType<typeof unwrapRootKeysForCompanion>>, privateKey: string) => void;
}) => ({
  ...
  encryptKeyPackage: async (requestedPublicKey: string) => {
    if (!options.primarySecrets || !options.primaryState || !options.primarySecurity?.isPinCreated) {
    ...
    return encodeCompanionKeyPackage(await wrapRootKeyForCompanion(
      rootKey,
      options.primaryState.accountId,
      requestedPublicKey,
      {
        keyEpoch: epoch,
  ...
  decryptAndPersist: async (privateKey: string, encrypted: Uint8Array, requestedPublicKey: string) => {
    const result = await unwrapRootKeysForCompanion(decodeCompanionKeyPackage(encrypted), requestedPublicKey, privateKey);
    options.onUnwrapped?.(result, privateKey);
  },
});

let webAccessTokenProvider: (() => Promise<string>) | null = null;
const optionsApi = () => createConfiguredSyncV2ApiClient(async () => {
...
export const requestSyncV2CompanionPairing = async (auth: WebGoogleSyncSession) => {
  ...
  if (!protocol.featureFlags.companionPairingEnabled) throw new Error('Sync V2 companion pairing is not enabled by the service.');
  const coordinator = new SyncV2PairingCoordinator(
    api, requestJournal(), pairingCrypto({}), transferAdapter(protocol.maximumSnapshotBytes), approvalJournal(),
  );
  const existing = await requestJournal().load();
  if (existing) {
    const remote = await api.getPairing(existing.pairingId, existing.requestedDeviceId).catch(() => null);
    if (!remote || remote.status === 'EXPIRED' || remote.status === 'REJECTED') await requestJournal().clear();
  }
...
export const getPendingSyncV2CompanionPairing = async (auth: WebGoogleSyncSession) => {
  ...
  return { pairing, pairingCode: pending.pairingCode, requestedDeviceId: pending.requestedDeviceId };
};

export const listPendingSyncV2Pairings = async (approverDeviceId: string): Promise<SyncV2Pairing[]> => {
...
export const approveSyncV2CompanionPairing = async (pairing: SyncV2Pairing, pairingCode: string): Promise<void> => {
  ...
  if (pairing.accountId !== state.accountId) throw new Error('Pairing request belongs to another Sync V2 account.');
  const api = createConfiguredSyncV2ApiClient(primaryAccessToken);
  const protocol = await api.getProtocol();
...
export const completeSyncV2CompanionPairing = async (
  ...
  if (remote.status === 'EXPIRED' || remote.status === 'REJECTED') throw new Error('Pairing request expired.');
  const protocol = await api.getProtocol();
  ...
  const coordinator = new SyncV2PairingCoordinator(
    api,
    requestJournal(),
    pairingCrypto({ onUnwrapped: (result, key) => { unwrapped = result; privateKey = key; } }),
    transferAdapter(protocol.maximumSnapshotBytes),
    approvalJournal(),
  );
  await coordinator.complete(async completed => {
    ...
    if (!keys.pinVerifier) {
      throw new Error('The companion package does not contain mobile PIN verification. Revoke it and pair again.');
    }
    ...
    const snapshots = new SyncV2SnapshotCoordinator(
      ...
      stateStore,
      new AccountKeySyncV2SnapshotCodec(async epoch => keys.accountRootKeys[epoch] || keys.accountRootKey),
    ...
    await diaryRepository.importSnapshot(repositorySnapshotFromV2(restored.state), 'replace-portable');
