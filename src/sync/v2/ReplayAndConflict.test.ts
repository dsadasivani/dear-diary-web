...
import { PersistentSyncConflictStore, SyncConflictResolutionService } from './conflict/PersistentSyncConflictStore';
...
const seedRuntime = (store: MemoryDataStore) => new SyncV2RuntimeStore(store).save({
...
const envelope = async (bytes: Uint8Array, patch: Partial<SyncV2RemoteEvent> = {}): Promise<SyncV2RemoteEvent> => ({
  sequence: 1, eventId: 'event-1', operationId: 'operation-remote-1', deviceId: 'device-2',
  recordType: 'NOTE', recordId: '22222222-2222-4222-8222-222222222222',
  operationType: 'UPSERT', recordVersion: 1, keyEpoch: 1, partitionKey: 'core',
  objectKey: 'object-remote-1', sha256: await sha256Hex(bytes), sizeBytes: bytes.byteLength,
  eventSchemaVersion: 2, downloadUrl: 'https://download.invalid/1',
  downloadExpiresAt: new Date().toISOString(), ...patch,
});

const decoded = (event: SyncV2RemoteEvent) => ({
  accountId: 'account-1', operationId: event.operationId, recordType: event.recordType,
  recordId: event.recordId, operationType: event.operationType,
  recordVersion: event.recordVersion, keyEpoch: event.keyEpoch,
...
test('remote pull survives a lost cursor acknowledgment without replaying the event', async () => {
  ...
  const api = {
    pullEvents: async (after: number) => after === 0
  ...
  await assert.rejects(puller.pull(), (error: unknown) => error instanceof SyncError && error.code === 'REQUEST_TIMEOUT');
...
test('replay rejects an invalid batch without advancing the atomic cursor', async () => {
  ...
  const second = await envelope(bytes, {
    sequence: 2, eventId: 'event-2', operationId: 'operation-2', recordVersion: 3,
  });
  await assert.rejects(replay.applyBatch([
...
test('integrity failure persists safety stop and does not advance replay', async () => {
  ...
  const puller = new RemoteEventPuller(
    ...
    new BoundedObjectTransfer({ maximumObjectBytes: 1024, fetch: async () => new Response(new Uint8Array([9, 9, 9])) }),
  ...
  await assert.rejects(puller.pull(), (error: unknown) => error instanceof SyncError && error.code === 'HASH_MISMATCH');
...
const localOperation = (patch: Partial<SyncOutboxOperationV2> = {}): SyncOutboxOperationV2 => ({
  ...
  recordType: 'NOTE', recordId: '33333333-3333-4333-8333-333333333333',
  operationType: 'UPSERT', baseRecordVersion: 0, state: 'CONFLICT', retryCount: 0,
...
test('persistent conflicts are stable and keep-local resolution enqueues a normal operation', async () => {
  ...
  const service = new SyncConflictResolutionService(conflicts, outbox, {
    create: async conflict => localOperation({
