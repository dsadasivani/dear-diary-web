...
export class PersistentWorkflowJournalStore<T> implements WorkflowJournalStore<T> {
  constructor(private readonly storage: LocalDataStore, private readonly key: string) {}
  async load(): Promise<T | null> {
    const raw = await this.storage.getItem(this.key);
    return raw ? JSON.parse(raw) as T : null;
  }
  save(value: T): Promise<void> { return this.storage.setItem(this.key, JSON.stringify(value)); }
  clear(): Promise<void> { return this.storage.removeItem(this.key); }
}
...
export class SyncV2MigrationCoordinator {
  ...
  async run(deviceId: string): Promise<void> {
    ...
    if (!state) {
      ...
      const remote = await this.api.beginMigration({ migrationId, deviceId, baselineDigest, baselineSequence });
    ...
    while (state.status !== 'V1_READ_ONLY') {
      if (state.status === 'FAILED' || state.status === 'ROLLED_BACK') throw new Error(`Migration stopped in ${state.status}.`);
  ...
  private async nextMigrationState(state: MigrationJournal): Promise<SyncV2MigrationStatus> {
    switch (state.status) {
      case 'PRECHECK': return 'DRAINING_V1';
      case 'DRAINING_V1': await this.local.drainV1(); return 'VALIDATING_LOCAL_STATE';
      case 'VALIDATING_LOCAL_STATE':
        state.validationDigest = await this.local.canonicalDigest();
        if (state.validationDigest !== state.baselineDigest) throw new Error('V1 changed while migration was draining.');
        return 'CREATING_V2_SNAPSHOT';
      case 'CREATING_V2_SNAPSHOT': state.snapshotId = (await this.local.createSnapshot()).snapshotId; return 'UPLOADING_V2_SNAPSHOT';
      case 'UPLOADING_V2_SNAPSHOT': return 'REGISTERING_V2_ACCOUNT';
      case 'REGISTERING_V2_ACCOUNT': return 'VERIFYING_V2_RESTORE';
      case 'VERIFYING_V2_RESTORE':
        if (!state.snapshotId || await this.local.verifyTemporaryRestore(state.snapshotId) !== state.baselineDigest) {
          throw new Error('V2 restore verification failed.');
        }
        return 'V2_ACTIVE';
      case 'V2_ACTIVE': return 'V1_READ_ONLY';
      default: throw new Error(`Unsupported migration state ${state.status}.`);
    }
  }
}
...
type PairingApi = Pick<SyncV2ApiClient,
...
export class SyncV2PairingCoordinator {
  ...
  async approve(input: {
    pairingId: string; requestedDeviceId: string; requestedPublicKey: string; challenge: string;
    pairingCode: string; approverDeviceId: string; keyEpoch: number;
  }): Promise<void> {
    ...
    if (journal && journal.pairingId !== input.pairingId) throw new Error('Another pairing approval is in progress.');
    if (!journal) {
...
interface RecoveryJournal { attemptId: string; deviceId: string; privateKeyHandle: string; keyPersisted: boolean }
type RecoveryApi = Pick<SyncV2ApiClient,
  'beginRecovery' | 'approveRecovery' | 'getRecoveryPackage' | 'markRecoveryKeyPersisted' | 'finalizeRecovery' | 'acknowledgeCursor'
>;
...
export class SyncV2RecoveryCoordinator {
  ...
  async run(deviceId: string, platform: string): Promise<void> {
    ...
    if (!state) {
      ...
      state = { attemptId: crypto.randomUUID(), deviceId, privateKeyHandle: key.privateKeyHandle, keyPersisted: false };
      await this.journal.save(state);
      await this.api.beginRecovery({ recoveryAttemptId: state.attemptId, recoveryDeviceId: deviceId, recoveryDevicePublicKey: key.publicKey, platform });
    }
    ...
    const proof = await this.local.sign(state.privateKeyHandle, `recovery-key-persisted:${state.attemptId}:${restored.snapshotId}`);
    await this.api.markRecoveryKeyPersisted(state.attemptId, {
      recoveryDeviceId: state.deviceId, validationSnapshotId: restored.snapshotId, possessionSignature: proof,
    });
...
type RotationApi = Pick<SyncV2ApiClient,
  ...
  | 'commitRotationEpoch' | 'markRotationLocalCommitted' | 'getRotation'
>;
...
export class SyncV2RotationCoordinator {
  constructor(
    ...
    private readonly local: {
      ...
      packageForTarget(encryptedKeyHandle: string, targetDeviceId: string, purpose: 'DEVICE' | 'RECOVERY'): Promise<Uint8Array>;
      upload(bytes: Uint8Array, instruction: SyncV2UploadInstruction): Promise<void>;
  ...
  async run(deviceId: string): Promise<void> {
    ...
    if (!state.packagesUploaded) {
      if (!state.packages) {
        const targets: Array<{ targetDeviceId: string; purpose: 'DEVICE' | 'RECOVERY' }> =
          (await this.local.activeDeviceIds()).map(targetDeviceId => ({ targetDeviceId, purpose: 'DEVICE' }));
        targets.push({ targetDeviceId: await this.local.recoveryTargetDeviceId(), purpose: 'RECOVERY' });
        state.packages = [];
        for (const target of targets) {
          const bytes = await this.local.packageForTarget(state.encryptedKeyHandle, target.targetDeviceId, target.purpose);
          state.packages.push({
            ...target,
            keyPackageId: crypto.randomUUID(),
            encryptedBase64: toBase64(bytes),
    ...
    remote = await this.api.getRotation(state.rotationId);
    if (remote.status === 'NEW_KEY_CREATED') remote = await this.api.advanceRotation(state.rotationId, state.deviceId, 'KEY_PACKAGES_CREATED');
    if (remote.status === 'KEY_PACKAGES_CREATED') remote = await this.api.advanceRotation(state.rotationId, state.deviceId, 'SERVER_EPOCH_PENDING');
    ...
    if (remote.status === 'SERVER_EPOCH_COMMITTED') {
      ...
      const proof = await this.local.sign(`rotation-local-committed:${state.rotationId}:${state.toEpoch}`);
  ...
  private async uploadPackage(state: RotationJournal, keyPackage: RotationPackageJournal): Promise<void> {
    const bytes = fromBase64(keyPackage.encryptedBase64);
    const initiated = await this.api.initiateKeyPackage({
      keyPackageId: keyPackage.keyPackageId, creatorDeviceId: state.deviceId,
      targetDeviceId: keyPackage.targetDeviceId, keyEpoch: state.toEpoch,
      purpose: keyPackage.purpose, sha256: keyPackage.sha256, sizeBytes: bytes.byteLength,
      packageSchemaVersion: 1, rotationId: state.rotationId,
    });
