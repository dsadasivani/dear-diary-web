...
export interface BoundedTransferOptions {
  ...
  resumableUploader?: (
    ...
    instruction: SyncV2UploadInstruction,
  ) => Promise<void>;
  fetch?: typeof fetch;
  telemetry?: Telemetry;
}
...
export class BoundedObjectTransfer {
  ...
  upload(objects: TransferObject[], instructions: SyncV2UploadInstruction[]): Promise<void[]> {
    ...
    return mapBounded(objects, this.concurrency, async (object) => {
      ...
      const response = await this.fetcher(instruction.uploadUrl, {
        method: 'PUT',
        headers,
        body: object.bytes,
      });
      if (!response.ok)
        throw new SyncError({ code: 'OBJECT_UPLOAD_FAILED', retryable: response.status >= 500 });
      span.end();
    });
  }
  ...
  download<T extends { downloadUrl: string; sizeBytes: number; sha256: string }>(
    ...
    return mapBounded(objects, this.concurrency, async (object) => {
      ...
      if (!response.ok)
        throw new SyncError({ code: 'OBJECT_DOWNLOAD_FAILED', retryable: response.status >= 500 });
