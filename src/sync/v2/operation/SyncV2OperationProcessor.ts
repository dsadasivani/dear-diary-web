...
const patchWithoutState = (patch: Partial<SyncOutboxOperationV2> & { state: SyncOutboxStateV2 }) => {
...
export class SyncV2OperationProcessor {
  ...
  constructor(
    ...
    private readonly api: Pick<SyncV2ApiClient, 'initiateOperation' | 'commitOperation' | 'getOperation'>,
  ...
  async runOnce(): Promise<boolean> {
    ...
    const span = this.telemetry.startSpan('outbox.operation', {
      operation_type: claimed.operationType, record_type: claimed.recordType, outbox_state: claimed.state,
    });
    try {
      this.validator.validateOperation(claimed, this.options.accountId, this.options.deviceId);
      await this.process(claimed);
      this.telemetry.counter('deardiary.sync.push.success', 1, { operation_type: claimed.operationType });
      span.end();
    } catch (error) {
      ...
      this.telemetry.counter('deardiary.sync.push.failure', 1, { error_code: typed.code, retryable: typed.retryable });
      if (typed.code === 'INVARIANT_VIOLATION') this.telemetry.counter('deardiary.sync.integrity.invariant_failure', 1);
      span.end(typed.code);
  ...
  private async process(initial: SyncOutboxOperationV2): Promise<void> {
    ...
    if (operation.state === 'RETRY_WAIT') {
      const resumeState: SyncOutboxStateV2 = operation.remoteSequence !== undefined
        ? 'COMMITTED'
        : operation.encryptedEventObjectKey ? 'UPLOADING' : 'PREPARING';
      operation = await this.outbox.transition(
        operation.operationId, 'RETRY_WAIT', resumeState, {}, this.options.workerId,
      );
    }
    if (operation.state === 'PENDING') {
      operation = await this.outbox.transition(
        operation.operationId, 'PENDING', 'PREPARING', {}, this.options.workerId,
      );
    }
    ...
    if (operation.state === 'PREPARING' || operation.state === 'UPLOADING') {
      ...
      if (operation.state === 'PREPARING') {
        operation = await this.outbox.transition(operation.operationId, 'PREPARING', 'UPLOADING', {
      ...
      const objects = await Promise.all(prepared.objects.map(async object => ({
      ...
      operation = await this.outbox.transition(
        operation.operationId, 'UPLOADING', 'READY_TO_COMMIT', {}, this.options.workerId,
      );
    }

    if (operation.state === 'READY_TO_COMMIT') {
      operation = await this.outbox.transition(
        operation.operationId, 'READY_TO_COMMIT', 'COMMITTING', {}, this.options.workerId,
      );
    }
    if (operation.state === 'COMMITTING') {
      const result = await this.commitOrReconcile(operation.operationId);
      operation = await this.outbox.transition(operation.operationId, 'COMMITTING', 'COMMITTED', {
  ...
  private async commitOrReconcile(operationId: string): Promise<SyncV2CommitResult> {
    ...
    try {
      ...
      if (status.status === 'COMMITTED' && status.sequence !== null && status.recordVersion !== null) {
        reconcileSpan.end();
        return { status: status.status, operationId, sequence: status.sequence, recordVersion: status.recordVersion };
      }
  ...
  private async handleFailure(operation: SyncOutboxOperationV2, error: SyncError): Promise<void> {
    ...
    if (failure.state === 'CONFLICT') {
      await this.conflicts.record(current, current.remoteRecordVersion || current.baseRecordVersion + 1);
    }
