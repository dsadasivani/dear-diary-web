...
test('filters repository snapshots by partition', () => {
  ...
  assert.deepEqual(
    july.entries.map((entry) => entry.id),
    ['entry-1'],
  );
  assert.deepEqual(
    july.notes.map((note) => note.id),
    ['note-1'],
  );
...
test('round-trips partition snapshot payloads', () => {
  ...
  assert.deepEqual(
    parsed.snapshot.entries.map((entry) => entry.id),
    ['entry-1'],
  );
...
test('builds and round-trips encrypted-manifest payload input', () => {
  ...
  const parsed = parsePartitionManifestPayload(
  ...
  assert.deepEqual(
    parsed.partitions.map((partition) => partition.partitionKey),
    ['core', 'month:2026-07', 'month:2021-03'],
  );
  assert.equal(
    parsed.partitions.find((partition) => partition.partitionKey === 'month:2026-07')?.mediaCount,
    1,
  );
});
