...
const SENSITIVE_KEY = /(token|secret|pass|pin|answer|content|body|title|tag|mood|path|url|uri|payload|accountid|deviceid|fileid|recordid|objectkey|partitionkey|error)$/i;
const MAX_SAFE_STRING_LENGTH = 80;

const sanitizeValue = (value: unknown): unknown => {
  if (value === null || typeof value === 'boolean' || typeof value === 'number') return value;
  if (typeof value === 'string') return value.length <= MAX_SAFE_STRING_LENGTH ? value : '[redacted]';
