export const TELEMETRY_ATTRIBUTES = [
  'platform', 'app_version', 'protocol_version', 'storage_backend', 'operation_type',
  'record_type', 'outbox_state', 'error_code', 'retryable', 'retry_count_bucket',
  'outbox_depth_bucket', 'sequence_lag_bucket', 'payload_size_bucket', 'network_type',
  'device_role', 'sync_mode',
] as const;

export type TelemetryAttributeName = typeof TELEMETRY_ATTRIBUTES[number];
export type TelemetryAttributes = Partial<Record<TelemetryAttributeName, string | number | boolean>>;
...
export class PrivacySafeTelemetry implements Telemetry {
  ...
  counter(name: string, value: number, attributes?: TelemetryAttributes): void { this.record('counter', name, value, attributes); }
  histogram(name: string, value: number, attributes?: TelemetryAttributes): void { this.record('histogram', name, value, attributes); }
  gauge(name: string, value: number, attributes?: TelemetryAttributes): void { this.record('gauge', name, value, attributes); }
  event(name: string, level: TelemetryLevel, attributes?: TelemetryAttributes): void {
    this.enqueue({ kind: 'event', name: this.validateName(name), level, attributes: this.attributes(attributes), sessionId: this.sessionId, at: this.now() });
  }
  startSpan(name: string, attributes?: TelemetryAttributes): TelemetrySpan {
    ...
    return {
      ...
      end: errorCode => this.enqueue({
        kind: 'span', name: this.validateName(name), attributes: this.attributes({
          ...spanAttributes,
          ...(errorCode ? { error_code: errorCode } : {}),
        }), sessionId: this.sessionId, at: startedAt, durationMs: Math.max(0, this.now() - startedAt),
      }),
    };
  }
  ...
  async flush(): Promise<void> {
    ...
    try { await this.exporter.export(batch); } catch { this.queue.unshift(...batch); }
    if (this.queue.length > 0) this.scheduleFlush();
  }

  private record(kind: TelemetryEnvelope['kind'], name: string, value: number, attributes?: TelemetryAttributes): void {
    if (!Number.isFinite(value)) return;
    this.enqueue({ kind, name: this.validateName(name), value, attributes: this.attributes(attributes), sessionId: this.sessionId, at: this.now() });
  }
  ...
  private scheduleFlush(): void {
    if (!this.flushTimer) this.flushTimer = setTimeout(() => void this.flush(), this.flushIntervalMs);
  }
  ...
  private attributes(attributes: TelemetryAttributes = {}): TelemetryAttributes {
    return Object.fromEntries(Object.entries(attributes)
      .filter(([key, value]) => (TELEMETRY_ATTRIBUTES as readonly string[]).includes(key) && ['string', 'number', 'boolean'].includes(typeof value))) as TelemetryAttributes;
  }
}
...
export class HttpTelemetryExporter implements TelemetryExporter {
  ...
  async export(envelopes: TelemetryEnvelope[]): Promise<void> {
    const response = await this.fetcher(this.endpoint, {
      ...
      body: JSON.stringify({ envelopes }), keepalive: true,
    });
...
export const NOOP_TELEMETRY: Telemetry = {
  counter: () => undefined, histogram: () => undefined, gauge: () => undefined,
  event: () => undefined,
