...
export default function DiarySettingsScreen({ diary, layout = 'mobile', security, onBack, onRefreshDiaries }: DiarySettingsScreenProps) {
  ...
  const openDelete = async () => {
    ...
    setPin(''); setDeleteConfirmation(''); setError(''); setDeleteOpen(true); setBusy(false);
  };

  const remove = async () => {
    if (!deleteNameMatches) { setError(`Type “${diary.name}” exactly to confirm deletion.`); return; }
    if (requiresDeletePin && !verifyPin(security, pin)) { setError('Incorrect app PIN.'); return; }
    setBusy(true);
  ...
  const sectionNavigation = (vertical: boolean) => <nav aria-label="Journal settings sections" className={vertical ? 'sticky top-24 h-fit' : 'overflow-x-auto pb-1'}>
    <p className={vertical ? 'mb-2 px-3 text-xs font-bold uppercase tracking-[0.16em] text-brand-text-muted' : 'sr-only'}>Settings sections</p>
  ...
  return <div className="mx-auto w-full max-w-6xl space-y-5 pb-12">
    ...
    <div className={layout === 'mobile' ? 'space-y-5' : layout === 'desktop' ? 'grid grid-cols-[168px_minmax(0,1fr)_280px] items-start gap-6' : 'grid grid-cols-[minmax(0,1fr)_280px] items-start gap-6'}>
      {layout === 'desktop' && sectionNavigation(true)}
      <div className="space-y-5">
        <section id="journal-settings-general" aria-labelledby="journal-settings-general-title" className="surface-elevated scroll-mt-28 space-y-4 p-5"><h2 id="journal-settings-general-title" className="font-serif-diary text-xl font-semibold">Name and privacy</h2><label className="block text-sm font-bold">Journal name<input value={draft.name} onChange={event => setDraft(current => ({ ...current, name: event.target.value }))} className="mt-2 w-full rounded-xl border border-brand-border bg-brand-card-bg px-4 py-3 text-base" /></label><label className="flex min-h-11 items-center gap-3 text-sm font-bold"><input type="checkbox" checked={draft.isLocked} onChange={event => setDraft(current => ({ ...current, isLocked: event.target.checked }))} className="h-5 w-5" /><Lock className="h-4 w-4" />Require unlock before opening</label></section>
        <section id="journal-settings-appearance" aria-labelledby="journal-settings-appearance-title" className="surface-elevated scroll-mt-28 p-5"><button type="button" aria-expanded={appearanceOpen} aria-controls="journal-settings-appearance-content" onClick={() => { setAppearanceOpen(value => !value); setActiveSection('appearance'); }} className="flex w-full items-center justify-between text-left"><span><span id="journal-settings-appearance-title" className="block font-serif-diary text-xl font-semibold">Appearance</span><span className="text-xs text-brand-text-muted">Cover, color, emoji, and foil</span></span><span>{appearanceOpen ? 'Hide' : 'Edit'}</span></button>{appearanceOpen && <div id="journal-settings-appearance-content" className="mt-5 space-y-5">
        ...
        <section id="journal-settings-danger" aria-labelledby="journal-settings-danger-title" className="scroll-mt-28 rounded-2xl border border-red-300 bg-red-50/70 p-5 dark:bg-red-950/20"><h2 id="journal-settings-danger-title" className="font-serif-diary text-xl font-semibold text-red-800 dark:text-red-200">Delete journal</h2><p className="mt-2 text-sm text-red-700 dark:text-red-200">Deletes this journal and its downloaded entries and media from this device. If Sync is connected, deletion is also synchronized.</p><AppButton className="mt-4" tone="danger" onClick={() => { setActiveSection('danger'); void openDelete(); }} disabled={busy}><Trash2 className="h-4 w-4" />Review deletion</AppButton></section>
    ...
    <AppDialog open={deleteOpen} title={`Delete ${diary.name}?`} description="Review exactly what will be removed before continuing." onClose={closeDeleteDialog} footer={<><AppButton onClick={closeDeleteDialog}>Cancel</AppButton><AppButton tone="danger" data-testid="confirm-delete-journal-button" onClick={() => void remove()} disabled={busy || !deleteNameMatches || (requiresDeletePin && !pin)}>Delete journal</AppButton></>}>
      <StatusNotice tone="warning">{counts.entries} entries · {counts.photos} photos · {counts.audio} audio recordings</StatusNotice>
      <label className="mt-4 block text-sm font-bold" htmlFor="delete-journal-name-confirmation">Type <strong>{diary.name}</strong> to confirm
        <input id="delete-journal-name-confirmation" data-testid="delete-journal-name-confirmation" type="text" autoComplete="off" spellCheck={false} value={deleteConfirmation} onChange={event => { setDeleteConfirmation(event.target.value); setError(''); }} className="mt-2 w-full rounded-xl border border-brand-border bg-brand-card-bg px-4 py-3" />
      </label>
      {requiresDeletePin && <label className="mt-4 block text-sm font-bold">App PIN<input type="password" inputMode="numeric" autoComplete="current-password" value={pin} onChange={event => { setPin(event.target.value); setError(''); }} className="mt-2 w-full rounded-xl border border-brand-border bg-brand-card-bg px-4 py-3" /></label>}{error && <StatusNotice className="mt-3" tone="danger" role="alert">{error}</StatusNotice>}
    </AppDialog>
  </div>;
}
