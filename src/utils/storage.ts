...
const normalizeEmoji = (value: string | undefined, fallback: string): string => {
  if (!value) return fallback;
  return EMOJI_REPLACEMENTS[value] || value;
};
...
// Seed images
const DotomboriImg =
  "https://images.unsplash.com/photo-1542051841857-5f90071e7989?w=600&auto=format&fit=crop&q=60";
const OkonomiyakiImg =
  "https://images.unsplash.com/photo-1590401830605-bf0f1e82845c?w=600&auto=format&fit=crop&q=60";
const TeaJournalImg =
  "https://images.unsplash.com/photo-1517842645767-c639042777db?w=600&auto=format&fit=crop&q=60";
const RainWindowImg =
  "https://images.unsplash.com/photo-1437419764001-2399a24d81b2?w=600&auto=format&fit=crop&q=60";
const StonesImg =
...
// TODO(capacitor): migrate these synchronous calls to src/platform/storage LocalDataStore
// once screens can load/save asynchronously without changing current behavior.
const STORAGE_KEYS = {
...
const sanitizeStoredEmojiData = () => {
  try {
    const diaries: Diary[] = JSON.parse(localStorage.getItem(STORAGE_KEYS.DIARIES) || "[]");
    const sanitizedDiaries = diaries.map((diary) => ({
      ...diary,
      emoji: normalizeEmoji(diary.emoji, "📔"),
    }));
    if (JSON.stringify(diaries) !== JSON.stringify(sanitizedDiaries)) {
      setStorageItem(STORAGE_KEYS.DIARIES, JSON.stringify(sanitizedDiaries));
    }

    const entries: Entry[] = JSON.parse(localStorage.getItem(STORAGE_KEYS.ENTRIES) || "[]");
    const sanitizedEntries = entries.map((entry) => ({
      ...entry,
      moodEmoji: normalizeEmoji(entry.moodEmoji, "💭"),
    }));
    if (JSON.stringify(entries) !== JSON.stringify(sanitizedEntries)) {
      setStorageItem(STORAGE_KEYS.ENTRIES, JSON.stringify(sanitizedEntries));
    }

    const profile = JSON.parse(localStorage.getItem(STORAGE_KEYS.USER_PROFILE) || "null") as UserProfile | null;
    if (profile) {
      const sanitizedProfile = {
        ...profile,
        avatarEmoji: normalizeEmoji(profile.avatarEmoji, "🌸"),
      };
      if (profile.avatarEmoji !== sanitizedProfile.avatarEmoji) {
        setStorageItem(STORAGE_KEYS.USER_PROFILE, JSON.stringify(sanitizedProfile));
      }
    }

    const settings = JSON.parse(localStorage.getItem(STORAGE_KEYS.SETTINGS) || "null") as AppSettings | null;
    if (settings?.customMoods) {
      const sanitizedSettings = {
        ...settings,
        customMoods: settings.customMoods.map((mood) => ({
          ...mood,
          emoji: normalizeEmoji(mood.emoji, "💭"),
        })),
      };
      if (JSON.stringify(settings) !== JSON.stringify(sanitizedSettings)) {
        setStorageItem(STORAGE_KEYS.SETTINGS, JSON.stringify(sanitizedSettings));
      }
    }
  } catch (error) {
    console.warn("Stored emoji sanitization failed:", error);
  }
};
...
export const saveUserProfile = (profile: UserProfile) => {
  ...
  try {
    ...
    if (user) {
      setDoc(doc(db, "users", user.uid), profile).catch((e) =>
        console.warn("Cloud sync profile failed:", e),
      );
    }
  } catch (err) {
    console.warn(err);
...
export const getSecurityConfig = (): SecurityConfig => {
  ...
  const config = JSON.parse(
    localStorage.getItem(STORAGE_KEYS.SECURITY) || "{}",
  );
...
export const saveAppSettings = (settings: AppSettings) => {
  ...
  try {
    const user = auth.currentUser;
    if (user) {
      setDoc(doc(db, "users", user.uid, "settings", "app"), settings).catch(
        (e) => console.warn("Cloud sync settings failed:", e),
      );
    }
  } catch (err) {
    console.warn(err);
...
export const createDiary = (
  ...
  try {
    ...
    if (user) {
      setDoc(
        doc(db, "users", user.uid, "diaries", newDiary.id),
        newDiary,
      ).catch((e) => console.warn("Cloud sync createDiary failed:", e));
    }
  } catch (err) {
    console.warn(err);
  }
...
export const updateDiary = (updatedDiary: Diary): Diary[] => {
  ...
  if (index !== -1) {
    ...
    try {
      ...
      if (user) {
        setDoc(
          doc(db, "users", user.uid, "diaries", updatedDiary.id),
          updatedDiary,
        ).catch((e) => console.warn("Cloud sync updateDiary failed:", e));
      }
    } catch (err) {
      console.warn(err);
    }
...
export const deleteDiary = (diaryId: string) => {
  ...
  try {
    const user = auth.currentUser;
    if (user) {
      deleteDoc(doc(db, "users", user.uid, "diaries", diaryId)).catch((e) =>
        console.warn("Cloud deleteDiary failed:", e),
      );
      for (const entry of entriesToDelete) {
        deleteDoc(doc(db, "users", user.uid, "entries", entry.id)).catch((e) =>
          console.warn("Cloud deleteEntry failed:", e),
        );
      }
    }
  } catch (err) {
...
export const createEntry = (
  entryData: Omit<
    Entry,
    "id" | "createdAt" | "updatedAt" | "wordCount" | "photoCount"
  >,
): Entry => {
  const entries = getEntries();
  const wordCount = entryData.body
    ? entryData.body.trim().split(/\s+/).filter(Boolean).length
    : 0;
  ...
  try {
    ...
    if (user) {
      setDoc(
        ...
        newEntry,
      ).catch((e) => console.warn("Cloud sync createEntry failed:", e));
    }
  } catch (err) {
    console.warn(err);
  }
...
export const updateEntry = (updatedEntry: Entry): Entry[] => {
  ...
  if (index !== -1) {
    const wordCount = updatedEntry.body
      ? updatedEntry.body.trim().split(/\s+/).filter(Boolean).length
      : 0;
    const finalEntry = {
      ...updatedEntry,
      wordCount,
    ...
    try {
      ...
      if (user) {
        setDoc(
          doc(db, "users", user.uid, "entries", finalEntry.id),
          finalEntry,
        ).catch((e) => console.warn("Cloud sync updateEntry failed:", e));
      }
    } catch (err) {
      console.warn(err);
    }
...
export const deleteEntry = (entryId: string) => {
  ...
  try {
    const user = auth.currentUser;
    if (user) {
      deleteDoc(doc(db, "users", user.uid, "entries", entryId)).catch((e) =>
        console.warn("Cloud deleteEntry failed:", e),
      );
    }
  } catch (err) {
...
export const createNote = (
  ...
  try {
    const user = auth.currentUser;
    if (user) {
      setDoc(doc(db, "users", user.uid, "notes", newNote.id), newNote).catch(
        (e) => console.warn("Cloud sync createNote failed:", e),
      );
    }
  } catch (err) {
    console.warn(err);
...
export const updateNote = (updatedNote: Note): Note[] => {
  ...
  if (index !== -1) {
    ...
    try {
      ...
      if (user) {
        setDoc(
          doc(db, "users", user.uid, "notes", finalNote.id),
          finalNote,
        ).catch((e) => console.warn("Cloud sync updateNote failed:", e));
      }
    } catch (err) {
      console.warn(err);
    }
...
export const deleteNote = (noteId: string) => {
  ...
  try {
    const user = auth.currentUser;
    if (user) {
      deleteDoc(doc(db, "users", user.uid, "notes", noteId)).catch((e) =>
        console.warn("Cloud deleteNote failed:", e),
      );
    }
  } catch (err) {
    console.warn(err);
...
export const updateDiaryEntryCounts = () => {
  const diaries = JSON.parse(
    localStorage.getItem(STORAGE_KEYS.DIARIES) || "[]",
  );
...
export const importEncryptedBackup = (
  ...
  try {
    ...
    if (
      ...
      !Array.isArray(parsedData.diaries) ||
    ...
    setStorageItem(
      STORAGE_KEYS.DIARIES,
      JSON.stringify(parsedData.diaries),
    );
    setStorageItem(
    ...
    setStorageItem(
      STORAGE_KEYS.NOTES,
      JSON.stringify(parsedData.notes || []),
    );
    ...
    if (parsedData.userProfile) {
      setStorageItem(
        STORAGE_KEYS.USER_PROFILE,
        JSON.stringify(parsedData.userProfile),
      );
    }
