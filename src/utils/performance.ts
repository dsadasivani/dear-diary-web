...
const PRIVATE_KEY_PATTERN =
  /(body|text|title|token|secret|key|pin|passphrase|password|recovery|answer|media|bytes|uri|html|content)/i;
...
export const setPerformanceTelemetry = (telemetry: Telemetry | null): void => {
  productionTelemetry = telemetry;
};

const telemetryMetricFor = (name: string): string | undefined => {
  if (name.startsWith('repository.local.') || name === 'repository.local.mutationWithOutbox')
    return 'deardiary.local_write.duration_ms';
  if (name === 'repository.initialize' || name.startsWith('sqlite.bridge'))
    return 'deardiary.database.open.duration_ms';
  ...
  if (name.includes('stats') || name.includes('Statistics'))
    return 'deardiary.screen.stats.duration_ms';
...
const emitProductionMeasurement = (name: string, durationMs: number, succeeded: boolean): void => {
  ...
  if (metric === 'deardiary.local_write.duration_ms') {
    productionTelemetry.counter(
      succeeded ? 'deardiary.local_write.success' : 'deardiary.local_write.failure',
      1,
    );
  }
  if (metric === 'deardiary.database.open.duration_ms') {
    productionTelemetry.counter(
      succeeded ? 'deardiary.database.open.success' : 'deardiary.database.open.failure',
      1,
    );
  }
};
...
const isEnabled = (): boolean => {
  ...
  if (typeof localStorage !== 'undefined' && localStorage.getItem('deardiary_perf') === 'off')
    return false;
  return true;
};
...
export const sanitizeMeasurementMetadata = (
  metadata?: MeasureMetadata,
): MeasureMetadata | undefined => {
...
export const measureSync = <T>(name: string, operation: () => T, metadata?: MeasureMetadata): T => {
...
export const getPerformanceAggregates = (): Record<
  string,
...
> =>
  Object.fromEntries(
    [...aggregates.entries()].map(([name, aggregate]) => {
