...
test('rejects a wrong passphrase and modified ciphertext', async () => {
  ...
  await assert.rejects(
    () => decryptBackupWithPassphrase(encrypted, 'this-password-is-wrong'),
    /incorrect|damaged/i,
  );
  const modified = encrypted.slice();
  modified[modified.length - 1] ^= 1;
  await assert.rejects(
    () => decryptBackupWithPassphrase(modified, password),
...
test('requires a strong-enough backup passphrase', async () => {
  await assert.rejects(
    () => encryptBackupWithPassphrase(new Uint8Array([1]), 'too-short'),
    /12 characters/i,
  );
});
