...
test('selects only backend CI for backend changes', () => {
  assert.deepEqual(
    classifyCiScope(['backend/sync-api/src/main/App.java']).scope,
...
test('selects only Android CI for native project changes', () => {
  assert.deepEqual(
    classifyCiScope(['android/app/build.gradle']).scope,
    expected({ android: true }),
  );
});
...
test('skips application jobs for documentation and operations changes', () => {
  assert.deepEqual(
    classifyCiScope([
      'docs/testing.md',
      'docs/supabase/018_migration.sql',
...
test('runs full CI for shared manifests and CI workflow changes', () => {
  const full = expected({ backend: true, web: true, browser: true, android: true });
...
test('fails safe to full CI for unclassified paths', () => {
  ...
  assert.deepEqual(
    result.scope,
    expected({ backend: true, web: true, browser: true, android: true }),
  );
});
