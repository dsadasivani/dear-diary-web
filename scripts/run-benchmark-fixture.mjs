...
const results = [
  ...
  measure('home.summary', () => {
    ...
    const recentEntries = [...entries].sort((left, right) => right.updatedAt - left.updatedAt).slice(0, 8);
    const pinnedNotes = notes.filter(note => note.isPinned).sort((left, right) => right.updatedAt - left.updatedAt).slice(0, 5);
