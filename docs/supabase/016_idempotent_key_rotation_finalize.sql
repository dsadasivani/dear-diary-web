-- Make key rotation finalization retry-safe after server-side completion.
-- A mobile force-stop can happen after finalize_device_key_rotation commits but
-- before the WebView receives or persists the response. Retrying should return
-- the finalized result instead of surfacing key_rotation_not_pending.
